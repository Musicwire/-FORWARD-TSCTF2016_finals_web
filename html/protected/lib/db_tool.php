...
class db_tool
{
    public function export($tables = array(), $save_dir = null, $save_name = null)
    {
        ...
        $content  = "# -------------------------------------------------------------\n";
        $content .= "# Dubhe Market Database Backup\n";
        $content .= "# Program: Dubhe Market ". $GLOBALS['Dversion']['VERSION'] . " Release " . $GLOBALS['Dversion']['RELEASE'] . "\n";
        $content .= "# MySql: {$db_version} \n";
        $content .= "# Database: {$GLOBALS['mysql']['MYSQL_DB']} \n";
        $content .= "# Creation: " . date('Y-m-d H:i:s') . "\n";
        $content .= "# Official: http://www.buptdubhe.com\n";
        $content .= "# -------------------------------------------------------------\n\n";
        ...
        if(empty($save_name)) $save_name = date('Ymd').'_'.time().'.php';
