...
{ 
    ...
    public function action_profile()
    {
        ...
        if(vds_request('step', null, 'get') == 'update')
        {
            ...
            if(TRUE === $verifier)
            {
				$update_data = " name = '".$data['name']."',gender = '".$data['gender']."',birth_year = '".$data['birth_year']."',birth_month = '".$data['birth_month']."',birth_day = '".$data['birth_day']."',mobile_no = '".$data['mobile_no']."',qq = '".$data['qq']."',signature = '".$data['signature']."' ";

                if($profile_model->update(" user_id = $user_id", $update_data,1) > 0)
                {
