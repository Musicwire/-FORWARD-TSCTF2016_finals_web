...
if(!($__controller))err("Err: Controller name '$__controller' is not correct!");
...
function inner_autoload($class){
	...
	foreach(array('model', 'lib', 'controller'.(empty($__module)?'':DS.$__module)) as $dir){
		...
		$file = APP_DIR.DS.'protected'.DS.$dir.DS.$class;
		if(is_file($file)){
			include $file;
			return;
		}
...
if(!($controller_name)) err("Err: Controller '$controller_name' is not exists!");
...
class Model{
	...
	public function update($conditions, $row ,$raw = 0){
		$values = array();
		if($raw == 1)
		{
			$sql = "UPDATE ".$this->table_name." SET". $row ."WHERE".$conditions;
